...
export class HintEntity {
    ...
    constructor(info = '验证完成后，你可以使用该邮箱登录或找回密码', color = '#c5c5c5', backgroundPosition = '0px -100px', imageName = 'icon.png') {
      this.info = info;
      this.color = color;
    ...
    static login(info: string): HintEntity {
      const result = new HintEntity(info);
